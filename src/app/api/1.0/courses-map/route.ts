...
const GRADE_RANK: Record<string, number> = { A: 7, AB: 6, B: 5, BC: 4, C: 3, D: 2, E: 1 };
...
export async function GET(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const c of courses ?? []) {
      ...
      if (typeof c.min_index === "string") c.min_index = c.min_index.trim() as any;
    }
    ...
    for (const row of enr ?? []) {
      ...
      if (g) {
        ...
        if (!prev || GRADE_RANK[up] > (GRADE_RANK[prev] ?? -1)) bestGradeByCourseId.set(cid, up);
      }
    }
    ...
    const coreqRes = await db.from("course_coreq").select("course_id, coreq_course_id");
    ...
    const rank = (s: Kelulusan) => (s === "Lulus" ? 3 : s === "Tidak Lulus" ? 2 : 1);
    ...
    for (const row of enr ?? []) {
      ...
      const courseSem = course.semester_no != null ? Number(course.semester_no) : null;
