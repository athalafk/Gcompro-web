...
function normalizeProbabilities(raw: unknown): Record<string, number> | undefined {
  ...
  if (typeof raw === 'object') {
    ...
    const nested = (obj as any).probabilities ?? (obj as any).probs ?? (typeof (obj as any).reasons === 'object' ? (obj as any).reasons : null);
    if (nested) return normalizeProbabilities(nested);
    const m: Record<string, number> = {};
    for (const [k, v] of Object.entries(obj)) if (typeof v === 'number') m[k] = v;
...
export async function GET(_req: NextRequest, context: { params: { id: string } }) {
  ...
  try {
    ...
    const { data: adv, error: advErr } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    if (advErr) return jsonPrivate({ error: advErr.message }, 500);
    ...
    return jsonPrivate({ feat, ai, meta }, 200);
  } catch (e: any) {
